...
## 1. 개요
- **목표:** RAG와 LoRA를 결합해 **고등학교 2학년 생명과학1** 학습지도안을 자동으로 생성하는 시스템을 설계한다. GPT 기반 코드 생성만으로 데이터 정제, 모델 학습, 검색 파이프라인, UI까지 구축 가능함을 전제로 한다.
- **대상:** 프로그래밍 경험이 적은 교사·연구자도 프롬프트 중심 워크플로로 손쉽게 활용할 수 있는 노코드/로우코드 구현.
- **범위:** 단일 과목(생명과학1), 단일 학년(고2)을 위한 지도안 자동화에 집중한다.
- **기대효과:** 지도안 작성 시간 90% 이상 단축, 교사별 문체·양식 반영, 교육 연구용 데이터 축적.

## 2. 핵심 기술
### 2.1 RAG (Retrieval-Augmented Generation)
- **정의:** LLM 생성 전에 외부 지식을 검색·주입하여 응답 정확도와 근거성을 높이는 방식.
- **용도:** 생명과학1 교과서, 교육과정, 기존 지도안 등에서 관련 문단을 검색해 생성 프롬프트에 포함.
- **예시:** “고2 생명과학1 광합성” 요청 시 성취기준·실험 절차 등 관련 문서를 자동 검색 후 LLM에 전달.

### 2.2 LoRA (Low-Rank Adaptation)
- **정의:** 대형 언어모델의 일부 가중치만 저랭크로 미세 조정해 개인화 스타일을 반영하는 방법.
- **용도:** 생명과학1 지도안 서식, 학교별 양식, 교사 고유 문체, 평가 방식 등을 반영한 맞춤형 모델 구축.
- **예시:** “서울시교육청 생명과학1 지도안 50개”로 LoRA 학습 후 동일한 문체·서식을 자동 생성.
...
| --- | --- | --- | --- |
| 데이터 수집 | Google Drive, Notion | 생명과학1 지도안·교육과정 문서 업로드 | 매우 쉬움 (노코드) |
| 검색 모듈 (RAG) | FlowiseAI / Dust.tt / ChatGPT 생성 코드 | 문서 로더→벡터스토어→리트리버→LLM | 쉬움 |
| 텍스트 생성 (LLM) | GPT-4 / Claude Sonnet | 생명과학1 지도안 본문 생성 | 매우 쉬움 |
...
## 4. 구현 단계
### 4.1 데이터 준비
1. **생명과학1** 학습지도안 샘플 30~100개 수집(고등학교 2학년에 한정).
2. 주제(예: 광합성, 세포 호흡, 유전), 세부 활동, 평가 요소를 열로 정리한 CSV 생성.
...
2. 프롬프트 템플릿 예시:
   ```
   다음 문서를 참고하여 아래 조건에 맞는 고등학교 2학년 생명과학1 학습지도안을 작성하시오.
   조건: {user_input}
   ```
...
| 주차 | 목표 | 산출물 |
| --- | --- | --- |
| 1주차 | 생명과학1 데이터 수집·정제, CSV 자동화 스크립트 확보 | 정제된 CSV, 변환 스크립트, 데이터 관리 가이드 |
